...
export async function markClaimAsFailed(publicId: string): Promise<void> {
  try {
    ...
    if (axios.isAxiosError(error) && error.response) {
      ...
      throw new Error(typeof data === 'object' ? JSON.stringify(data) : data || 'Failed to mark claim as failed');
    }
    throw new Error(
...
export async function markClaimAsFinished(publicId: string): Promise<void> {
  try {
    ...
    if (axios.isAxiosError(error) && error.response) {
      const { data } = error.response;
      throw new Error(typeof data === 'object' ? JSON.stringify(data) : data || 'Failed to mark claim as finished');
    }
    throw new Error(
